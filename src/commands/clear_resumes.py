...
def handle_clear_resumes_command(interaction_data):
    ...
    try:
        ...
        if not existing_resumes or len(existing_resumes) == 0:
            ...
            return "You don't have any resumes to clear. 📭"
        ...
        if not db_success:
            ...
            return "Failed to clear resume metadata from database. Please try again later. 😔"
        ...
        if not s3_success:
            ...
            return f"Successfully cleared {resume_count} resume records, but some files may remain in storage. Contact support if needed. ⚠️"
        
        logger.info(f"Clear workflow completed successfully for user {user_id}")
        return f"✅ Successfully cleared all {resume_count} of your resumes from the system. All data has been permanently deleted."
        ...
        return "An error occurred while clearing your resumes. Please try again later. 😔"